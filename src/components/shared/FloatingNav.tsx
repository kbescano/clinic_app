...
export default function FloatingNav({ navItems }: { navItems: FloatingNavItem[] }) {
  ...
  return (
    <>
      ...
      <nav
        className={`lg:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-black/95 backdrop-blur-md border-t border-gray-100 dark:border-white/15 z-[9999] pb-safe shadow-[0_-1px_10px_rgba(0,0,0,0.05)] transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${
            ...
            return (
              ...
              >
                <div className="relative">
                  <Icon
                    className={`w-5 h-5 transition-colors duration-200 ${isActive ? 'text-[#251101] dark:text-white' : 'text-gray-500'}`}
                  />
                </div>
                <span
                  className={`text-[10px] mt-1 font-bold uppercase tracking-wider ${isActive ? 'text-[#251101] dark:text-white' : 'text-gray-500'}`}
                >
                  {item.name}
                </span>
              </Link>
