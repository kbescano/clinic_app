...
export async function getBusySlots(dateString: string): Promise<string[]> {
  ...
  const busy = await payload.find({
    ...
    // Only the time window is read, and a busy day must never be cut off at a page limit.
    pagination: false,
    depth: 0,
...
export async function createBookingAction(
  ...
  // Read the verified session before resetting it: rescheduling must prove ownership with it.
  const sessionEmail = await getPatientSessionEmail()
  ...
  if (rescheduleId) {
    // The id comes from the client; only let a verified patient move their own appointment.
    const original = await payload
      .findByID({ collection: 'appointments', id: rescheduleId, depth: 0, overrideAccess: true })
      .catch(() => null)
    if (!original || !sessionEmail || original.email?.trim().toLowerCase() !== sessionEmail) {
  ...
  try {
    ...
    const existingAptsData = await payload.find({
      ...
      // Default page size is 10: truncating here would under-count overlaps and allow overbooking.
      pagination: false,
      depth: 0,
      overrideAccess: true,
    })
    ...
    if (mainPatient && mainPatient.email) {
      await setPatientSession(mainPatient.email)
    }
